...
class Tester_Qcm_Controller extends Controller
{
    ...
    public function insererResultatQcm(Request $request) {
        ...
        $totalPoints = 0;
        $linesCorrect = 0; // Variable pour suivre le nombre de lignes correctes
        ...
        for ($i = 0; $i < count($question); $i++) {
            try {
                $insererResultat = $qcm_result->insert($qcm[$i], $totalPoints); // Insérez les données dans la base de données ici
            } catch (\Exception $e) {
