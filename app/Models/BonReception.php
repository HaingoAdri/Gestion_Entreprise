...
class BonReception extends Model
{
    ...
    public function getAllReception(){
        $requete = " select * from bon_reception";
        $reponse = DB::select($requete);
        $liste = array();
        if(count($reponse)>0){
            foreach($reponse as $result){
                $bonReception = new BonReception(id: $result->id);
                $liste[] = $bonReception;
            }
        }
        return $liste;
    ...
    public function getTva($reception, $module, $article){
        $sql = "select tva from V_Details_Bon_Reception_details where id = '".$reception."' and id_module = ".$module." and article = '".$article."'";
