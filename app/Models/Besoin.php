...
class Besoin extends Model
{
    ...
    public function getUneBesoin($id) {
        ...
        if(count($reponse) > 0) {
            $besoin = new Besoin();
            $besoin->id = $reponse[0]->id;
            $besoin->idPoste = $reponse[0]->idposte;
            $besoin->idService = $reponse[0]->idservice;
            $besoin->poste = (new Poste())->getUnePoste($besoin->idPoste);
            $besoin->service = (new Service())->getUneService($besoin->idService);
            $besoin->besoin_horaire = $reponse[0]->besoin_horaire;
            $besoin->heure_jour_homme = $reponse[0]->heure_jour_homme;
            $besoin->description = $reponse[0]->description;
            $besoin->type_contrat = (new Type_Contrat())->getUnTypeContrat($reponse[0]->id_type_contrat);
            $besoin->personnes = ((int)($besoin->besoin_horaire/$besoin->heure_jour_homme));
            $besoin->annonce = (new Annonce())->createAnnonce($besoin->id);
        }
        return $besoin;
