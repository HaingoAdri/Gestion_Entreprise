...
class Qcm_Result extends Model {
    ...
    public function insert() {
        try {
            $requete = "insert into qcm_result(qcm, notes_r) values (".$this->qcm.", ".$this->notes_r.")";
            echo $requete;
            DB::insert($requete);
        } catch (Exception $e) {
    ...
    public function getUn_Qcm_Result_Par_id() {
        $requette = "select * from qcm_result where id_r = " . $this->id_r;
        $reponse = DB::select($requette);
        $Qcm_Result = null;
        if(count($reponse) > 0) {
            $Qcm_Result = new Qcm_Result();
