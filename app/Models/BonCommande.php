...
class BonCommande extends Model
{
    ...
    public function __construct($id = "", $date = "", $idPayement = "",  $delaiLivarison = "", $idProformat = "", $etat = "", $finDelai = "") {
        $this->id = $id;
        $this->date = $date;
    ...
    public function getListeLivraisonEnRetard() {
        $requette = "select * from livarison_attente where fin_delai < now()";
        $reponse = DB::select($requette);
        $liste = array();
        if(count($reponse) > 0){
            foreach($reponse as $resultat) {
                $bonCommande = new BonCommande(id: $resultat->id, date: $resultat->date, idPayement: $resultat->idpayement, delaiLivarison: $resultat->delailivarison, etat: $resultat->etat, finDelai: $resultat->fin_delai);
                $idDemande = $bonCommande->getIdDemande();
                $demande = (new Demande(idDemande: $idDemande))->getDonneesUnDemande();
                $bonCommande->nom = $demande->nom;
                $liste[] = $bonCommande;
